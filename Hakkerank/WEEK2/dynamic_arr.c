...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    int *seqSizes = calloc(n, sizeof(int));     // track size of each sequence
    int **seq = malloc(n * sizeof(int*));       // array of sequences

    for (int i = 0; i < n; i++) {
        seq[i] = malloc(1000 * sizeof(int));    // give each sequence some space
    }
    ...
    *result_count = 0;
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        int idx = (x ^ lastAnswer) % n;   // sequence index
